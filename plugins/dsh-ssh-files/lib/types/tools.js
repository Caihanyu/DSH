...
export function registerSshTools(ctx, store, caps) {
    ...
    tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const { server } = await store.ensureConnected(sessionId, typeof reference === 'string' ? reference : undefined, caps.connectTimeoutMs, exec.signal);
    ...
    tools.register(defineTool({
        ...
        async execute(args, exec) {
            const conn = store.requireConnectedFor(sessionKeyOf(exec));
            const input = args;
            const outcome = await conn.exec(input.command, 
    ...
    ctx.systemPrompt.section({
        name: 'tool:ssh-files',
        order: 100,
        text: 'SSH 工具（ssh_status / ssh_connect / ssh_list / ssh_read / ssh_write / ssh_mkdir / ssh_rm / ssh_exec）作用于本对话各自记住的服务器，与会话隔离：每个对话独立连接、互不影响。'
