...
import { SshConnection, type SshFileEntry, type SshListing } from './ssh.ts'
import {
  createServerId, freshPref, loadState, prefFor, saveState,
  type SessionPref, type SshMode, type SshServer, type SshState,
} from './state.ts'
...
/** One live session runtime: its working preference plus its own connection. */
interface SessionRuntime {
  conn: SshConnection
}

/** One local directory listing (mirrors the remote listing shape). */
async function listLocal(path: string): Promise<SshListing> {
...
async function readLocal(path: string, maxBytes: number): Promise<string> {
...
async function mkdirLocal(path: string): Promise<void> {
  await mkdir(path, { recursive: false })
}

/** Delete one local file or (empty) directory. */
async function unlinkLocal(path: string): Promise<void> {
  const stats = await stat(path)
...
function serverOf(state: SshState, serverId: string | null): SshServer | undefined {
...
export class SshSessionStore {
  ...
  private state: SshState | null = null
  private readonly runtimes = new Map<SessionKey, SessionRuntime>()
  ...
  private async current(): Promise<SshState> {
    await this.ready
  ...
  private runtimeFor(sessionId: SessionKey): SessionRuntime {
  ...
  private async writePref(sessionId: SessionKey, pref: SessionPref): Promise<void> {
  ...
  async setMode(sessionId: SessionKey, mode: SshMode): Promise<SshStateResponse> {
    const state = await this.current()
    const pref = prefFor(state, sessionId === '' ? undefined : sessionId)
  ...
  async addServer(input: SshServerInput, sessionId: SessionKey = ''): Promise<SshStateResponse> {
    const state = await this.current()
  ...
  async updateServer(id: string, input: SshServerInput, sessionId: SessionKey = ''): Promise<SshStateResponse> {
    const state = await this.current()
    const index = state.servers.findIndex(candidate => candidate.id === id)
    const previous = state.servers[index]
    if (previous === undefined) throw new Error('服务器记录不存在')
  ...
  async removeServer(id: string, sessionId: SessionKey = ''): Promise<SshStateResponse> {
    const state = await this.current()
    const index = state.servers.findIndex(candidate => candidate.id === id)
  ...
  async connect(
    sessionId: SessionKey, id: string, timeoutMs: number, signal: AbortSignal,
  ): Promise<SshStateResponse> {
    const state = await this.current()
    const server = state.servers.find(candidate => candidate.id === id)
    if (server === undefined) throw new Error('服务器记录不存在')
    const runtime = this.runtimeFor(sessionId)
  ...
  async disconnect(sessionId: SessionKey): Promise<SshStateResponse> {
    const state = await this.current()
    const runtime = this.runtimeFor(sessionId)
    await runtime.conn.disconnect()
    const pref = prefFor(state, sessionId === '' ? undefined : sessionId)
  ...
  async ensureConnected(
    sessionId: SessionKey, reference: string | undefined, timeoutMs: number, signal: AbortSignal,
  ): Promise<{ conn: SshConnection; server: SshServer }> {
    const state = await this.current()
    const runtime = this.runtimeFor(sessionId)
  ...
  async list(sessionId: SessionKey, path: string, signal?: AbortSignal): Promise<SshListing> {
    const state = await this.current()
    const pref = prefFor(state, sessionId === '' ? undefined : sessionId)
  ...
  async read(sessionId: SessionKey, path: string, maxBytes: number, signal?: AbortSignal): Promise<string> {
    const state = await this.current()
    const pref = prefFor(state, sessionId === '' ? undefined : sessionId)
  ...
  async write(sessionId: SessionKey, path: string, content: string, signal?: AbortSignal): Promise<void> {
    const state = await this.current()
    const pref = prefFor(state, sessionId === '' ? undefined : sessionId)
  ...
  async mkdir(sessionId: SessionKey, path: string, signal?: AbortSignal): Promise<void> {
    const state = await this.current()
    const pref = prefFor(state, sessionId === '' ? undefined : sessionId)
  ...
  async unlink(sessionId: SessionKey, path: string, signal?: AbortSignal): Promise<void> {
    const state = await this.current()
    const pref = prefFor(state, sessionId === '' ? undefined : sessionId)
